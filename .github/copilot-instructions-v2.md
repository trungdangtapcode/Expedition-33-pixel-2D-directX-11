...
## 4. Interface Pattern - Module dependency via pure virtual interfaces
- For any cross-cutting system (e.g., CutsceneSystem, QTESystem), define a pure virtual interface (e.g., `ICutsceneSystem`) that exposes only the necessary methods
- The concrete implementation (e.g., `CutsceneSystem`) is owned by `GameApp` and passed as a pointer to states that require it
- This decouples states from specific system implementations and allows for easier testing and future refactors
