# Overworld Tiled Map Integration

## Goal

The overworld map is authored as a fixed-size orthogonal Tiled JSON map
that can be opened directly in the Tiled editor and loaded by the game
without a conversion step.

Current map:

- Path: `assets/environments/overworld_map.json`
- Size: `128 x 96` tiles
- Tile size: `64 x 64` pixels
- World span: `8192 x 6144` pixels
- Runtime origin: the renderer centers the map on world `(0, 0)`
- Ground atlas: `assets/environments/overworld_tiles_v2.png`
- Object atlas: `assets/environments/overworld_objects_v2.png`

## Tiled Format Contract

The loader expects the standard Tiled JSON shape:

- Top-level map fields: `type`, `orientation`, `renderorder`, `width`,
  `height`, `tilewidth`, `tileheight`, `layers`, and `tilesets`.
- Tile layers: `type: "tilelayer"` with an integer `data` array.
- Empty tile cells use global tile id `0`.
- Tilesets use `firstgid` so the renderer can map global tile ids back
  into the correct atlas.
- Object layers can define collision rectangles through `x`, `y`,
  `width`, and `height`.

The runtime also supports external JSON tileset references through the
Tiled `source` field. Embedded tilesets remain the default for this map
because they keep the whole sample easy to inspect in one file.

## Editing Workflow

1. Open `assets/environments/overworld_map.json` in Tiled.
2. Keep the map orthogonal and fixed-size. Infinite maps are not part of
   the current renderer.
3. Keep tile layer data exported as JSON arrays. Base64 or compressed
   layer data is not supported by the hand-written parser.
4. Use background layers for base terrain, roads, low objects, and decals.
5. Use the `Collisions` object layer for solid AABB rectangles.
6. Use foreground layer names only for always-above art such as canopies.
7. Save the map as JSON.

## Runtime Notes

`TileMapRenderer` draws tile layers through `SpriteBatch` with
`camera.GetViewMatrix()`. SpriteBatch handles the pixel-to-NDC transform
internally, so passing a view-projection matrix would double-project the
tiles and make them disappear.

The renderer now supports two map passes. Normal visible tile layers render
behind `SceneGraph` entities. Layers whose names include `foreground`, `front`,
`above`, `canopy`, `roof`, or `overlay` render after entities, so authored maps
can place canopies, roof edges, and gate tops above the player without changing
C++.

The loader clears Tiled flip flag bits before atlas lookup. Flipped tile
rendering itself is not implemented yet, so avoid relying on flipped
tiles for authored content.

## Generators

The checked-in atlases are generated by:

```text
patches/compile_assets.py
```

The checked-in sample map and static prop data are generated by:

```text
patches/generate_map.py
```

Run both from the workspace root to rebuild the overworld:

```text
python patches/compile_assets.py
python patches/generate_map.py
```

The generator creates a directed story route rather than a plain test
field. The current layout contains a central meadow camp, silent market
ruins, western watch side path, pilgrim crossing, glass shrine, eastern
mirror gate, four campfire recovery points, eleven overworld encounter
placements, static props, map-edge boundaries, and matching collision
rectangles.

Related data files:

- `data/campfires.json` places the recovery and checkpoint campfires.
- `data/overworld_spawns.json` places overworld enemy encounters.
- `data/overworld_story.json` defines area names and objective text.
- `data/overworld_props.json` places large Y-sorted static props.
- `data/overworld_themes.json` defines region color grade themes.
- `docs/11-print-asset-machenism/overworld_render_pipeline.md` documents the
  runtime render pass and foreground layer naming contract.
- `docs/11-print-asset-machenism/overworld_visual_rebuild.md` documents the
  generated asset pipeline, static props, and theme rules.
