...
void SineSpawner::Update(float dt, float mBoxCx, float mBoxCy, float mBoxW, float mBoxH, float mHeartX, float mHeartY, std::vector<PhysicsBullet>& outBullets)
{
    mSpawnTimer += dt;
    ...
    while (mSpawnTimer > interval) {
        ...
        b.behavior = BulletBehavior::Sine;
        b.amplitude = mData.sineAmplitude;
        b.frequency = mData.sineFrequency;
        
        int side = rand() % 4;
        float speed = mData.bulletSpeed;
        
        // Same logic as RandomEdgeSpawner to pick starting edge
        if (side == 0) { // Top edge
        ...
        // Aim exactly at heart (base trajectory)
        float dx = mHeartX - b.x;
        float dy = mHeartY - b.y;
